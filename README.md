...
`SCOUT_DEV_MODE=true` (по умолчанию) — шаблоны и статика с диска, без пересборки.
...
Единый файл настроек: порт HTTP, тема, TLE, SatNOGS, координаты наблюдателя, радиотракты. Переменные окружения для прикладных настроек **не поддерживаются** — всё хранится в файле.
...
|------------|-------------|----------|
| `SCOUT_CONFIG_PATH` | `data/config.json` (локально) / `/app/data/config.json` (Docker) | Путь к файлу конфигурации |
| `SCOUT_DEV_MODE` | `true` локально, `false` в Docker | `true` — шаблоны/статика с диска; `false` — embed (production) |
